...
class Products with ChangeNotifier {
  ...
  Future<void> fetchAndSetProducts([bool filterByUser = false]) async{
    ...
    // & is used to filter the products by user's ID
    final filterString = filterByUser ? 'orderBy="creatorId"&equalTo="$userId"' : '';
    var url = 'https://shopapp-e1a4e.firebaseio.com/products.json?auth=$authToken&$filterString';
    try{
      ...
      url = 'https://shopapp-e1a4e.firebaseio.com/userFavorites/$userId.json?auth=$authToken';
  ...
  Future<void> updateProduct(String id, Product newProduct) async{
    ...
    if(prodIndex >= 0){
      final url = 'https://shopapp-e1a4e.firebaseio.com/products/$id.json?auth=$authToken';
      await http.patch(
          url,
          body: json.encode({
